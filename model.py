...
class MarketModel(mesa.Model):
    ...
    def __init__(self, params):
        ...
        self.datacollector = DataCollector(
                agent_reporters = {'day_profit': 'day_profit',
    ...
    def constrained_scheduler(self, unconstrained_schedule, period):
        ...
        for agent_id, dispatch in unconstrained_schedule.items():
            ...
            if agent.constraint_mode() == 'on':
                if dispatch == 0:
                    agent.run_time += 1
                    constrained_schedule[agent_id] = 1
                    self.period_offers[period][agent_id] = agent.marginal_cost
                else:
                    agent.run_time += 1
                    constrained_schedule[agent_id] = 1
        ...
        constrained_schedule = OrderedDict(sorted(constrained_schedule.items(), 
                                                  key = lambda x: (-x[1], self.period_offers[period][x[0]])))
        ...
        if 0 in satisfying_demand:
            self.system_marginal_cost[period] = self.period_offers[period][satisfying_demand.index(0)]
    ...
    def clear_market(self, offers, demand, period):
        ...
        accepted_offers = list(
                np.where(cumulative_offers <= demand_forecast, 
                         1, 0))
        ...
        if self.constraints == 'all':
            if accepted_offers[list(offers.keys()).index(0)] == 0:
                ...
                self.period_offers[period] = offers
...
#                
#            self.system_marginal_cost[period] = list(offers.items())[accepted_offers.index(0)][1][1]
...
#        else:
#            
#            self.system_marginal_cost[period] = list(self.period_offers[period].items())[-1][1][1]
#        
        ...
        if 0 in accepted_offers:
            ...
            # minimum and maximum generation
                ...
                and (marginal_gen.offer[period][3] <= marginal_volume <= marginal_gen.offer[period][2]):
                    
                accepted_offers[accepted_offers.index(0)] = marginal_volume/(marginal_gen.offer[period][0])
            ...
            elif (sum([offer[1][0] - offer[1][3] for offer in list(offers.items())[1:(accepted_offers.index(0))]]) >= (marginal_gen.offer[period][3] - marginal_volume)) \
                and (marginal_gen.offer[period][3] >= marginal_volume):
                
                accepted_offers[accepted_offers.index(0)] = marginal_gen.offer[period][3]/(marginal_gen.offer[period][0])
                
                # strictly the marginal volume is now negative, but we're keeping
                # it positive here for ease of comparison
                marginal_volume = marginal_gen.offer[period][3] - marginal_volume
    ...
    def step(self):
        ...
        for agent in self.schedule.agents:
            day_offers[agent.id] = deepcopy(agent.offer)
        ...
        self.period_offers = OrderedDict()
        self.system_marginal_cost = OrderedDict()
        self.imbalance = []
        ...
        for period in range(48):
            ...
            self.period_offers[period] = OrderedDict() 
    
            for gen_id in day_offers.keys():
                
                self.period_offers[period][gen_id] = day_offers[gen_id][period]
                
            # sorts by offer; if there is a tie, sorts by marginal cost    
            self.period_offers[period] = OrderedDict(
                    sorted(self.period_offers[period].items(), 
                           key = lambda x: (x[1][1], self.schedule.agents[x[0]].marginal_cost))) 
            ...
            accepted_offers = self.clear_market(self.period_offers[period], 
                                                     self.demand,
                                                     period)
            ...
            for i, agent in enumerate(list(self.period_offers[period].keys())):
...
#            elif self.constraints == 'none':
            ...
            for agent in self.schedule.agents:
                ...
                self.generation[period].update(
                        {agent.id: [agent.fuel,
                                    agent.offer[period][0] * self.constrained_schedule[period][agent.id],
                                    self.period_offers[period][agent.id][1]]})
        ...
        # as amended by any rational constraints.
        ...
        self.schedule.step(self.params['stage_list'][1], 
                           self.generation,
                           self.system_marginal_cost)
        
        
        # BALANCING MARKET CLEARING GOES HERE
        # should probably be sent to another discrete function also. bm_generation
        # currently not used. UPDATE STAGE PASSING 0 IN TIME.PY AS WELL
...
def run_simulation(model_class, params, days, show_graphs = False, save_graphs = False, iterate = False, name = None):
    ...
    day_profit = model.datacollector.get_agent_vars_dataframe()['day_profit']
    period_profit = model.datacollector.get_agent_vars_dataframe()['period_profit']
    ...
    offers = model.datacollector.get_agent_vars_dataframe()['offers']
    ...
    for period in tqdm(range(48), desc = 'Crunching data...'):
        ...
        for day in range(100):
            for agent_id in gen_labels.iloc[-1].keys():
                all_gen.iloc[day][agent_id] = generation.iloc[-day][period][agent_id][1]
    ...
    if show_graphs == True:
        ...
        for i in range(params['num_gen']):
            ax1.plot(range(days - 25), running_mean(day_profit[:,i], 25)/1000,
        ...
        for period in range(48):
            for i, (gen, dispatch) in enumerate(dispatch_schedules[days - 1][period]):
                ax2.bar(range(period, period+1),
                        ...
                        bottom = sum([generation.iloc[days-1][period][j][1] * 2 for j in [dispatch_schedules[days - 1][period][z][0] for z in range(i)]]))
        ax2.plot(model.demand, 'b-', linewidth = 2)
        ax2.set(title = 'Final Day Dispatch in Offer Price Order with Demand',
                xlabel = 'Iteration (Day)',
...
def batchrun_simulation(model_class, fixed_params, days, 
    ...
    for i, value in enumerate(results[results.columns[0]]):
        ...
        profit = results['Datacollector'].iloc[i].get_agent_vars_dataframe()['day_profit']
