...
class StagedActivation(BaseScheduler):
    ...
    ### we're going to modify this so that each model step only activates one
    ### action for all of the agents at a time, so that market-level stuff
    ### can be done between each action that would otherwise be cumbersome
    ### to implement on a per-agent basis. Additionally, we're going to allow
    ### arguments to be passed to agents at each step.
    
    # for now, this is tailored specifically to the Roth-Erev day-offers / 
    # accepted offers arguments, and will need changing in the future.

    def step(self, stage, generation = None, px_marginal_price = None, voll_flags = None):
        ...
        # Run stage. Bit choppy atm, only passing through the arguments if the
        ...
        for agent_key in agent_keys:
            if stage == 'make_px_offer':
                ...
                getattr(self._agents[agent_key], stage)(generation, px_marginal_price)
            elif stage == 'update_bm_propensities':
                getattr(self._agents[agent_key], stage)(generation, voll_flags)
            else:
                pass
